    ...
    public async ValueTask<Unit> Handle(HashedGeniusCommand command, CancellationToken cancellationToken)
    {
        ...
        do
        {
            ...
            var signature_verified = Ed25519Adapter.Instance.Verify(hash.Hash,
                ...
                Ed25519Adapter.Import(new Core.GeoCryptoKey { IsPrivate = false,
                    KeyType =  Core.GeoCryptoKey.CryptoKeyType.Ed25519,
            ...
            if (command.Request.Current.SealedEnvelope != null)
            {
                ...
                var privateKey = (X25519PrivateKeyParameters)X25519Adapter.Import(new Core.GeoCryptoKey { IsPrivate = true, 
                    Key = Convert.ToBase64String(bob_key) });


                var adapter = (X25519PrivateKeyParameters)X25519Adapter.Import(CosmosAdapter.ToCosmos(command.Request.Current.SealedEnvelope)!);
                var secret = new byte[32];
                privateKey.GenerateSecret(adapter.GeneratePublicKey(), secret, 0);
                ...
                var decrypted = AesAdapter.GcmDecryptData(Convert.FromBase64String(command.Request.Current.SealedEnvelope.Data), hkdf_key,
                    Encoding.UTF8.GetBytes(command.Request.Current.SealedEnvelope.Nonce), Convert.FromBase64String(command.Request.Current.SealedEnvelope.Tag));
