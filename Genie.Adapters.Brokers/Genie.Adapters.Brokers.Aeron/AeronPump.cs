...
public sealed class AeronPump<T>
{
    ...
    /// </summary>
    public AeronSubscription MessageQueue { get; }
    ...
    /// </summary>
    private AeronPump(AeronSubscription messageQueue, Func<byte[], Task> processMessage, int maxDegreeOfParallelism, CancellationToken ct)
    {
            ...
            Task producer = Task.Run(async () =>
            {
                try
                {
                    while (Stop1.Task.Status != TaskStatus.RanToCompletion)
                    {
                        ...
                        MessageQueue.ReceiveSignal.WaitOne();
                        await buffer.SendAsync(MessageQueue.Data).ConfigureAwait(false);
