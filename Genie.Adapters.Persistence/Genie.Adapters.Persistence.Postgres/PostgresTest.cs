...
public class PostgresTest(int payload, ObjectPool<PostgresPooledObject> pool) : PersistenceTestBase, IPersistenceTest
{
    public int Payload { get; set; } = payload;

    readonly ObjectPool<PostgresPooledObject> Pool = pool;

    public PostgresTest() : this(JsonSize,new DefaultObjectPool<PostgresPooledObject>(new DefaultPooledObjectPolicy<PostgresPooledObject>()))
    {

    }

    public PostgresTest(bool create) : this(JsonSize,new DefaultObjectPool<PostgresPooledObject>(new DefaultPooledObjectPolicy<PostgresPooledObject>()))
    {
        if (create)
            CreateDB();
    }
    ...
    public void CreateJsonDB()
    {
        ...
        cmd.CommandText = "CREATE TABLE json_data(id text NOT NULL, json JSON NOT NULL, CONSTRAINT PK_bench PRIMARY KEY (id))";
        cmd.ExecuteNonQuery();
    ...
    public override void CreateIndex()
    {
        ...
        using var command = lease.DataSource.CreateCommand("CREATE INDEX idx_postal ON country_postal(postal_code);");
        command.ExecuteNonQuery();
    }
    ...
    public override bool WriteJson(long i)
    {
        ...
        try
        {
            ...
            cmd.Parameters.Add(new NpgsqlParameter { ParameterName = "json", Value = JsonSerializer.Serialize(test), NpgsqlDbType = NpgsqlDbType.Json });
            cmd.ExecuteNonQuery();
        }
        catch(Exception ex)
        {
    ...
    public override bool ReadJson(long i)
    {
        ...
        PostgresPooledObject lease = null;
