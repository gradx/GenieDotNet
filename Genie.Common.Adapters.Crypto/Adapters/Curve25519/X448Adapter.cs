...
// Key Agreement
public class X448Adapter : IAsymmetricBase, IAsymmetricCipher<HkdfParameters>
{
    private static readonly Lazy<X448Adapter> _instance = new(() => new());
    private X448Adapter() { }
    public static X448Adapter Instance { get { return _instance.Value; } }
    ...
    {
        var genX = new X448KeyPairGenerator();
    ...
    {
        if (k.IsPrivate)
            return new X448PrivateKeyParameters(k.X509, 0);
        else
    ...
    {
        return new X448PublicKeyParameters(new X509Certificate2(x509).GetPublicKey(), 0);
    }

    public static byte[] GenerateSecret(X448PrivateKeyParameters privateKey, X448PublicKeyParameters publicKey)
    {
    ...
    {
        if (key is X448PrivateKeyParameters parameters)
