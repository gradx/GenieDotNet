...
public sealed class RabbitMQPump<T>
{
    ...
    /// </summary>
    public static RabbitMQPump<T> Run(IBasicConsumer messageQueue, Func<BasicDeliverEventArgs, Task> processMessage, int maxDegreeOfParallelism,
        CancellationToken ct = default)
    {
        if (messageQueue is not EventingBasicConsumer && messageQueue is not AsyncEventingBasicConsumer)
            throw new ArgumentException("messageQueue must be EventingBasicConsumer or AsyncEventingBasicConsumer");
    ...
    /// </summary>
    public IBasicConsumer MessageQueue { get; }
    ...
    /// </summary>
    private RabbitMQPump(IBasicConsumer messageQueue, Func<BasicDeliverEventArgs, Task> processMessage, int maxDegreeOfParallelism, CancellationToken ct)
    {
    ...
    private async Task RunAsync(Func<BasicDeliverEventArgs, Task> processMessage, CancellationToken ct = default)
    {
        using CancellationTokenSource producerCTS = ct.CanBeCanceled ? CancellationTokenSource.CreateLinkedTokenSource(ct) : new CancellationTokenSource();
        {
            ...
            Task producer = Task.Run(async () =>
            {
                try
                {
                    if (MessageQueue is EventingBasicConsumer e)
                        e.Received += async (sender, ea) =>
                        {
                            WaitHandle.Set();
                            await buffer.SendAsync(ea, ct).ConfigureAwait(false);
                        };

                    else if (MessageQueue is AsyncEventingBasicConsumer ae)
                        ae.Received += async (sender, ea) =>
                        {
                            WaitHandle.Set();
                            await buffer.SendAsync(ea, ct).ConfigureAwait(false);
                        };
