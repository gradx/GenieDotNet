    ...
    public class Composite
    {
        ...
        public static async Task OpTest(PersistenceTestBase test)
        {
            ...
            var pump = CountryTimerPump.Run(Seconds, async message =>
            {
                ...
                if (join_response)
                {
                    if (CounterConsoleLogger.Counter < SampleSize)
                        update_response = true;
                    else
                    {
                        random.Next(1, SampleSize);
                        message.PlaceName = $@"Tagged by {message.Id}";
                        message.Id = random.Next(1, SampleSize);
                        update_response = await test.UpdatePostal(message);
                    }
                }

                stopwatch.Stop();

                if (!write_response || !read_response || !query_response || !join_response || !update_response)
                    logger.ProcessError();
