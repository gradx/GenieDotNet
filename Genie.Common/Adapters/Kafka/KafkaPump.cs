...
public sealed class KafkaPump<K, V>
{
    ...
    /// </summary>
    public IConsumer<K, V> MessageQueue { get; }
    ...
    /// </summary>
    private KafkaPump(IConsumer<K, V> messageQueue, Func<Message<K, V>, Task> processMessage, int maxDegreeOfParallelism, CancellationToken ct)
    {
    ...
    /// Pump implementation.
    /// </summary>
    private async Task RunAsync(Func<Message<K, V>, Task> processMessage, CancellationToken ct = default)
    {
        using CancellationTokenSource producerCTS = ct.CanBeCanceled ? CancellationTokenSource.CreateLinkedTokenSource(ct) : new CancellationTokenSource();
        {
            ...
            BufferBlock<Message<K, V>> buffer = new(bufferOptions);
            ...
            Task producer = Task.Run(async () =>
            {
                try
                {
                    while (Stop1.Task.Status != TaskStatus.RanToCompletion)
                    {
                        ...
                        var result = MessageQueue.Consume(ct);

                        await buffer.SendAsync(result.Message, ct).ConfigureAwait(false);
                    }
                }
                finally
                {
            ...
            ActionBlock<Message<K, V>> consumer = new(async message =>
            {
                ct.ThrowIfCancellationRequested();
