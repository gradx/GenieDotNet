...
public partial class CosmosAdapter
{
    ...
    public static Grpc.PartyBenchmarkRequest FromCosmos(PartyBenchmarkRequest req)
    {
        return new Grpc.PartyBenchmarkRequest
        {
            Request = FromBaseRequest(req),
            Party = CosmosAdapter.FromCosmos(req.Party!)
        };
    }

    public static Grpc.BaseRequest FromBaseRequest<T>(T request) where T : BaseRequest, new()
    {
        var t = new Grpc.BaseRequest
        {
            CosmosBase = new Grpc.CosmosBase
            {
                Attachments = request._attachments,
                Created = request.Created?.ToTimestamp(),
                Etag = request._etag,
            ...
            HorizontalAccuracy = request.HorizontalAccuracy ?? 0,
            Info = request.Info,
            IpAddressDestination = request.IPAddressDestination,
            IpAddressSource = request.IPAddressSource,
            Length = request.Length, 
            Origin = CosmosAdapter.FromCosmos(request.Origin!), 
            RequestTtl = request.RequestTtl?.ToTimestamp(),
            Type = request.Type,
            VerticalAccuracy = request.VerticalAccuracy ?? 0,
        };
