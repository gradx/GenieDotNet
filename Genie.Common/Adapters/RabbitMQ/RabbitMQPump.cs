...
public sealed class RabbitMQPump<T>
{
    ...
    /// </summary>
    private RabbitMQPump(AsyncEventingBasicConsumer messageQueue, Func<RabbitMessage, Task> processMessage, int maxDegreeOfParallelism, CancellationToken ct)
    {
    ...
    /// Pump implementation.
    /// </summary>
    private async Task RunAsync(Func<RabbitMessage, Task> processMessage, CancellationToken ct = default)
    {
        using CancellationTokenSource producerCTS = ct.CanBeCanceled ? CancellationTokenSource.CreateLinkedTokenSource(ct) : new CancellationTokenSource();
        {
            ...
            ActionBlock<RabbitMessage> consumer = new(async message =>
            {
                ct.ThrowIfCancellationRequested();
