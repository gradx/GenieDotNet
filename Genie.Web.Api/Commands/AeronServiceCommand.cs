...
public record AeronServiceCommand(ObjectPool<AeronServicePooledObject<EventTaskJob>> GeniePool, SchemaBuilder SchemaBuilder, ILogger<Exception> Logger, bool FireAndForget) : IRequest<Unit>;
...
public class AeronServiceCommandHandler(GenieContext genieContext) : BaseCommandHandler(genieContext), IRequestHandler<AeronServiceCommand, Unit>
{
    public async ValueTask<Unit> Handle(AeronServiceCommand command, CancellationToken cancellationToken)
    {
        try
        {
            AeronServicePooledObject<EventTaskJob> pooledObj = command.GeniePool.Get();
