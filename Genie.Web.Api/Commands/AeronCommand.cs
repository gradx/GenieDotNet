...
public record AeronCommand(ObjectPool<AeronPooledObject<EventTaskJob>> GeniePool, SchemaBuilder SchemaBuilder, ILogger<Exception> Logger, bool FireAndForget) : IRequest<Unit>;
...
public class AeronCommandHandler(GenieContext genieContext) : BaseCommandHandler(genieContext), IRequestHandler<AeronCommand, Unit>
{
    public async ValueTask<Unit> Handle(AeronCommand command, CancellationToken cancellationToken)
    {
        try
        {
            AeronPooledObject<EventTaskJob> pooledObj = command.GeniePool.Get();
