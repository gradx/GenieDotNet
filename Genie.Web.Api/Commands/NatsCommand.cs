...
public record NatsCommand(ObjectPool<NatsPooledObject<EventTaskJob>> GeniePool, SchemaBuilder SchemaBuilder, ILogger<Exception> Logger, bool FireAndForget) : IRequest<Unit>;
...
public class NatsCommandHandler(GenieContext genieContext) : BaseCommandHandler(genieContext), IRequestHandler<NatsCommand, Unit>
{
    public async ValueTask<Unit> Handle(NatsCommand command, CancellationToken cancellationToken)
    {
        try
        {
            NatsPooledObject<EventTaskJob> pooledObj = command.GeniePool.Get();
