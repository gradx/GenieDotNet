 ...
 public class Game(int credits)
{
    ...
    public async Task GetLicense()
    {
        ...
        static GeniusEventRequest GetMockLicenseRequest()
        {
            ...
            var channelKey = Ed25519Adapter.Import(new Genie.Core.GeoCryptoKey
            {
                Key = Convert.ToBase64String(File.ReadAllBytes(@"Keys\Alice\Ed25519SigningAdapter.key")),
                KeyType =  Genie.Core.GeoCryptoKey.CryptoKeyType.Ed25519,
                IsPrivate = true
            });
    ...
    {
        var ephemeral_key = X25519Adapter.GenerateKeyPair();
        ...
        {
            Key = Convert.ToBase64String(X25519Adapter.Export(ephemeral_key.Private)),
        ...
        var envelope = AesAdapter.GcmEncryptData(geniune, hdkfKey, Encoding.UTF8.GetBytes(nonce));
